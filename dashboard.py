"""
F-Droid Search Metrics Dashboard
"""
...
from search.analyzer import SearchMetricsAnalyzer


def main():
    """Main dashboard application."""
    st.set_page_config(
        page_title="F-Droid Search Metrics", page_icon="📊", layout="wide"
    )

    st.title("📊 F-Droid Search Metrics Dashboard")
    st.markdown(
        "Analyze search patterns and usage statistics from the F-Droid app store."
    )
    ...
    if not available_dates:
        st.error("No data files found. Please run the data collection script first.")
        return
    ...
    with tab1:
        show_overview(analyzer, selected_dates)

    with tab2:
        show_query_analysis(analyzer, selected_dates)

    with tab3:
        show_geographic_analysis(analyzer, selected_dates)

    with tab4:
        show_technical_analysis(analyzer, selected_dates)


def show_overview(analyzer: SearchMetricsAnalyzer, dates: list):
    ...
    if len(dates) == 1:
        ...
        if not ts_data.empty:
            ...
            fig = make_subplots(
                rows=2,
                cols=1,
                subplot_titles=("Daily Hits", "Daily Unique Queries"),
...
def show_query_analysis(analyzer: SearchMetricsAnalyzer, dates: list):
    ...
    col1, col2 = st.columns(2)
    with col1:
        min_hits = st.number_input("Minimum total hits", min_value=0, value=10)
    with col2:
        top_n = st.selectbox("Show top N queries", [10, 20, 50, 100], index=1)
...
def show_geographic_analysis(analyzer: SearchMetricsAnalyzer, dates: list):
    ...
    if not known_countries.empty:
        ...
        fig = px.bar(
            ...
            x="total_hits",
            y="country",
            title="Top 20 Countries by Total Hits",
...
def show_technical_analysis(analyzer: SearchMetricsAnalyzer, dates: list):
